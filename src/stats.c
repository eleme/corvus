...
static void stats_send(char *metric, double value)
{
    ...
    n = snprintf(NULL, 0, fmt, config.cluster_name, hostname, config.bind, metric, value);
    char buf[n + 1];
    snprintf(buf, sizeof(buf), fmt, config.cluster_name, hostname, config.bind, metric, value);
